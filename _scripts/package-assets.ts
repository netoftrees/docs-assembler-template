...
async function main() {
  ...
  const manifestPath = path.join(distDir, 'distribution.json');
  
  const manifest: SourceManifest = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
  ...
  const stageDir = path.join(ROOT, '.staging');
  ...
  for (const item of manifest.contents) {
    ...
    const destPath = path.join(stageDir, item.dest);
  ...
  for (const item of manifest.contents) {
    const stagedPath = path.join(stageDir, item.dest);
    const exists = await fs.access(stagedPath).then(() => true).catch(() => false);
    if (!exists) continue;
    ...
    if (item.type === 'file') {
      ...
      for await (const filePath of walkFiles(stagedPath)) {
        const relativePath = path.relative(stageDir, filePath).replace(/\\/g, '/');
        internalManifest.files.push({
          path: relativePath,
          strategy,
          mergeType: item.mergeType,
  ...
  console.log('\n🗜️  Zipping...');
  await createZip(stageDir, versionZip);
