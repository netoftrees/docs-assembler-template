...
async function updateReferences(assets: AssetFile[], searchDirs: string[]): Promise<void> {
    ...
    const patterns = assets.map(asset => {
        ...
        return { regex, replacement: asset.hashedName };
    });
...
async function main() {
    ...
    const targetDir = path.join(ROOT, 'docs', 'assets', 'FragmentRenderer');
    ...
    for (const asset of assets) {
        await fs.copyFile(asset.fullPath, path.join(targetDir, asset.hashedName));
    }
    console.log(`   ✓ ${assets.length} files copied`);

    // Update Jekyll references
    console.log(`\n🔄 Updating _includes and _layouts...`);
    await updateReferences(assets, ['docs/_includes', 'docs/_layouts']);
